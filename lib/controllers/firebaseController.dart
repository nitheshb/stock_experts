  ...
  Future<void> updateUserToken(userID, token, oldToken) async {
    ...
    await Firestore.instance
      ...
      for (final e in joinedGroups) {
        ...
        Firestore.instance.collection('groups').document(e).updateData({
          'FdeviceTokens': FieldValue.arrayUnion([token]),
          'FdeviceTokens': FieldValue.arrayRemove([oldToken])
        });
      }
      ...
      for (final e in followingGroups0) {
        ...
        var currentElement = e;
        Firestore.instance.collection('groups').document(e).updateData({
          'AlldeviceTokens': FieldValue.arrayUnion([token]),
          'AlldeviceTokens': FieldValue.arrayRemove([oldToken])
        });
      }
    });
  }
  ...
  Future<void> logout_updateUserToken(userID, token) async {
    ...
    await Firestore.instance
      ...
      for (final e in joinedGroups) {
        ...
        var currentElement = e;
        Firestore.instance.collection('groups').document(e).updateData({
          'FdeviceTokens': FieldValue.arrayRemove([token])
        });
      }
      ...
      for (final e in followingGroups0) {
        ...
        var currentElement = e;
        Firestore.instance.collection('groups').document(e).updateData({
          'AlldeviceTokens': FieldValue.arrayRemove([token])
        });
      }
    });
  }
...
//      }
  ...
  Future<QuerySnapshot> lookForExistingUserName(pickedUserName) async {
    var userName = await Firestore.instance
        .collection('IAM')
        .where("firstName", isEqualTo: pickedUserName)
        .getDocuments();
    return userName;
  }
  ...
  sendChatMessage(chatId, body, lastMessageBody, type) async {
    ...
    final dbRef1 =
        FirebaseDatabase.instance.reference().child("Notify").child(chatId);
    if (type == "Prime") {
      dbRef1.update({
        'pm': "${lastMessageBody["lastPmMsg"] ?? ''}",
        'pc': lastMessageBody["msgFullPmCount"] ?? 0,
      });
      Firestore.instance
          .collection('Chats')
          .document("${chatId}PGrp")
          .updateData({
        'lastMessageDetails': lastMessageBody,
        'messages': FieldValue.arrayUnion([body])
      });
    } else if (type == "Non-Prime") {
      dbRef1.update({
        'm': "${lastMessageBody["lastMsg"] ?? ''}",
        'c': lastMessageBody["msgFullCount"] ?? 0,
      });
  ...
  sendToClear121Message(chatId, body, lastMessageBody, type) async {
    ...
    if (type == "Prime") {
      ...
      Firestore.instance
          ...
          .document("${chatId}PGrp")
          .updateData(
              {'lastMessageDetails': lastMessageBody, 'messages': body});
    } else if (type == "Non-Prime") {
      dbRef1.update({
        'm': "${lastMessageBody["lastMsg"] ?? ''}",
        'c': lastMessageBody["msgFullCount"] ?? 0,
  ...
  userFirstVote(groupId, defaultBody) {
    Firestore.instance
        ...
        .document(groupId)
        .updateData({
      'VotingStats': FieldValue.arrayUnion([defaultBody])
    });
  }
  ...
  votingAvailableMatches(
    groupCategoriesArray,
  ) {
    return Firestore.instance
        .collection('Matches')
        .where("category", arrayContainsAny: groupCategoriesArray)
        .where("status", isEqualTo: "FbStart")
        .snapshots();
  }
...
// create a new group PAGE 
  createGroup(
    ...
    await Firestore.instance
        .collection('GroupSearch')
        .document('groupSearch${groupSearchTag}')
        .updateData({
      'payload': FieldValue.arrayUnion([
        {
          "title": groupTitle,
          "ownerName": firstName,
          "createdBy": userId,
          ...
          "FeeDetails": [
            {"fee": _premiumPrice1, "days": _premiumDays1}
          ],
        }
      ])
    });
    await Firestore.instance
        .collection("PrimeGroups")
        .document("${check1.documentID}")
        .setData(body);
    await Firestore.instance
        .collection("Devices")
    ...
    await Firestore.instance
        .collection("Chats")
        .document("${check1.documentID}PGrp")
        .setData(body);
    final dbRef1 = await FirebaseDatabase.instance
        .reference()
        .child("Notify")
        .child(check1.documentID);
    dbRef1.set({
      'm': "",
      't': "${body['title']}",
      'c': 0,
  ...
  unfollowGroup(chatId, userId, userToken) {
    Firestore.instance.collection('groups').document(chatId).updateData({
      'followers': FieldValue.arrayRemove([userId]),
      'AlldeviceTokens': FieldValue.arrayRemove([userToken])
    });
    Firestore.instance.collection('IAM').document(userId).updateData({
      'followingGroups0': FieldValue.arrayRemove([chatId])
    });
  ...
  followGroup(chatId, userId, userToken) async {
    ...
    Firestore.instance.collection('IAM').document(userId).updateData({
      'followingGroups0': FieldValue.arrayUnion([chatId])
    });
    Firestore.instance.collection('groups').document(chatId).updateData({
      'followers': FieldValue.arrayUnion([userId]),
      'AlldeviceTokens': FieldValue.arrayUnion([userToken])
    });
  }
...
// used to check if group values already exists while creating a new group
  searchResultsByName(query) {
    ...
    return Firestore.instance
        .collection('GroupSearch')
        .document('groupSearch${searchTag}')
        .snapshots();
  }
...
// get kYc content
  getUserKycStatus(chatId, userId) {
    return Firestore.instance
        ...
        .where("approve_status", isEqualTo: 'Review_Waiting')
        .snapshots();
  }
  ...
  submitKycDoc(body, userId, chatId) async {
    ...
    userTableDocRef.updateData({
      'WaitingGroups': FieldValue.arrayUnion([chatId]),
      'WaitingGroupsJson': FieldValue.arrayUnion([body])
    });
    return;
  }
  ...
  getKycDocsList(chatId) {
    return Firestore.instance
        .collection('KYC')
        .where("chatId", isEqualTo: chatId)
        .where("payment_approve_status", isEqualTo: 'Review_Waiting')
        .snapshots();
  }

  approveKycDoc(kycDocId, modifiedDate, period, userId, chatId, userToken,
      phoneNumber, firstName) {
    Firestore.instance.collection('KYC').document(kycDocId).updateData({
      'payment_approve_status': "Approved",
      'ApprovedOn': DateTime.now(),
      'expiresOn': modifiedDate,
      'membershipDuration': period,
      'reviewBy': userId
    });
    Firestore.instance.collection('IAM').document(userId).updateData({
      'approvedGroups': FieldValue.arrayUnion([chatId]),
      'approvedGroupsJson': FieldValue.arrayUnion([
        {'chatId': chatId, 'kycDocId': kycDocId}
      ]),
      'WaitingGroups': FieldValue.arrayRemove([chatId])
    });
    var groupUserBody = {
      'userId': userId,
      'joinedId': DateTime.now(),
      'expiresOn': modifiedDate,
      'membershipDuration': period,
      'kycDocId': kycDocId,
    ...
    Firestore.instance.collection('PrimeGroups').document(chatId).updateData({
      'premiumMembers': FieldValue.arrayUnion([userId]),
      'approvedGroupsJson': FieldValue.arrayUnion([groupUserBody]),
      'FdeviceTokens': FieldValue.arrayUnion([userToken]),
      'AlldeviceTokens': FieldValue.arrayRemove([userToken]),
      'rejectedId': FieldValue.arrayRemove([userId])
    });
  }

  rejectKycDoc(kycDocId, modifiedDate, period, userId, chatId) {
    Firestore.instance.collection('KYC').document(kycDocId).updateData(
        {'payment_approve_status': "Rejected", 'rejectedOn': DateTime.now()});
    Firestore.instance.collection('IAM').document(userId).updateData({
      'rejectedGroups': FieldValue.arrayUnion([chatId]),
      'rejectedGroupsJson': FieldValue.arrayUnion([chatId]),
      'WaitingGroups': FieldValue.arrayRemove([chatId])
    });
    Firestore.instance.collection('PrimeGroups').document(chatId).updateData({
      'rejectedId': FieldValue.arrayUnion([userId]),
      'rejectedIdJson': FieldValue.arrayUnion([userId]),
      'WaitingCount': 100
    });
  }

  removeMemberOnExpiry(
      userId, joinedTime, expiredTime, kycDocId, period, chatId, fullBody) {
    Firestore.instance.collection('IAM').document(userId).updateData({
      'approvedGroups': FieldValue.arrayRemove([chatId]),
      'approvedGroupsJson': FieldValue.arrayRemove([
        {'chatId': chatId, 'kycDocId': kycDocId}
      ]),
      'expiredGroups': FieldValue.arrayUnion([
        {'chatId': chatId, 'kycDocId': kycDocId}
      ])
    });
    var groupUserBody = {
    ...
    Firestore.instance.collection('PrimeGroups').document(chatId).updateData({
      'premiumMembers': FieldValue.arrayRemove([userId]),
      'approvedGroupsJson': FieldValue.arrayRemove([fullBody]),
      'expiredMembers': FieldValue.arrayUnion([userId])
    });
  }
