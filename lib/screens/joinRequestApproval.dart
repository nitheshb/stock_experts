...
import 'package:stock_experts/widgets/chat_bubble.dart';
...
class _JoinRequestApprovalState extends State<JoinRequestApproval>
  ...
  updatePaymentRequestStatus(
      kycDocId, userId, chatId, status, period, phoneNumber, firstName) async {
    ...
    if (status == "Rejected") {
      ...
      try {
        var response = await dio.get(
            "https://asia-south1-royalpro.cloudfunctions.net/onMembershipRejected?id=${userId}&chatId=${chatId}&groupName=${widget.groupName}");
      } catch (e) {}
    } else if (status == 'Approved') {
      var now = new DateTime.now();
      var modifiedDate = now.add(Duration(days: 30));
      FirebaseController.instanace.approveKycDoc(kycDocId, modifiedDate, period,
          userId, chatId, userToken, phoneNumber, firstName);
      try {
        var response = await dio.get(
            "https://asia-south1-royalpro.cloudfunctions.net/onMemberAdd?id=${userId}&chatId=${chatId}&groupName=${widget.groupName}");
      } catch (e) {}
    }
  }
  ...
  Widget _buildCircularBtn(
      double height, String img, int type, Function onTapFun) {
    double imageSize;
  ...
  Widget durationTag() {
    return Expanded(
      child: Container(
        decoration: BoxDecoration(
            color:
                true ? Bid365AppTheme.nearlyBlue : Bid365AppTheme.nearlyWhite,
